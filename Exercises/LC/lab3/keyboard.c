...
int (kb_subscribe_int)(uint8_t* bit_no, int* hook_id) {
  ...
  *hook_id = *bit_no;
  return sys_irqsetpolicy(KB_IRQ_VEC, KB_IRQ_POL, hook_id);
}
...
int (kb_next)(KB_State *state) {
  ...
  if (util_sys_inb(KB_STAT_REG, &state->status) != 0) {
  ...
  if (util_sys_inb(KB_OUT_BUF, &state->code[0]) != 0) {
  ...
  if (state->code[0] == 0xE0) {
    if (util_sys_inb(KB_OUT_BUF, &state->code[1]) != 0) {
      printf("[ERROR] could not read output buffer");
      return ERR_OUT_BUF;
    }
    state->size = 2;
...
int (kb_enable_int)(void) {
  uint8_t cmd = 0;
  if (util_sys_outb(KB_STAT_REG, KB_READ_COM) != 0) {
    printf("[ERROR] could not write to status register");
    return ERR_OUT_BUF;
  }
  if (util_sys_inb(KB_OUT_BUF, &cmd) != 0) {
    printf("[ERROR] could not read output buffer");
    return ERR_OUT_BUF;
  }
  cmd |= BIT(0);
  if (util_sys_outb(KB_STAT_REG, KB_WRITE_COM) != 0) {
    printf("[ERROR] could not write to status register");
    return ERR_OUT_BUF;
  }
  if (util_sys_outb(KB_OUT_BUF, cmd) != 0) {
