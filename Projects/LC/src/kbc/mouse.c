/**
 * @file mouse.c
 * @brief Implementation of the driver functions related to the mouse driver.
 * @authors Inês Almeida, João Ferreira
 */
...
int (ms_sample_rate)(uint8_t rate) {
  uint8_t byte = 0;
  uint8_t status = 0;
  if (kbc_write_cmd(MS_WRITE_BYTE, &status)) return 1;
  if (kbc_write_inbuf(MS_SR, &status)) return 1;
  if (kbc_read_outbuf(&byte, &status)) return 1;
  if (byte != MS_ACK) return 1;
  if (kbc_write_cmd(MS_WRITE_BYTE, &status)) return 1;
  if (kbc_write_inbuf(rate, &status)) return 1;
  if (kbc_read_outbuf(&byte, &status)) return 1;
  if (byte != MS_ACK) return 1;
  return OK;
}
