/**
 * @file video.c
 * @brief Implementation of the functions related to the video card driver.
...
int (initialize_video_memory) (VG_State* v) {
  ...
  v->delta_buffer = malloc(vram_size);
  return 0;
}

int (draw_pixel) (VG_State* v, uint16_t x, uint16_t y, uint32_t color) {
  if (x >= v->video_info.XResolution || y >= v->video_info.YResolution) {
    printf("[ERROR] draw_pixel(v, %d, %d, %d) is out of bounds.\n", x, y, color);
    return 1;
  }
  
  unsigned bytes_per_pixel = (v->video_info.BitsPerPixel + 7) / 8;  // Rounded up
  uint8_t* memory_position = &v->delta_buffer[(v->video_info.XResolution * y + x) * bytes_per_pixel];
...
void (vg_flush) (VG_State* v) {
  int vram_size = (v->video_info.XResolution * v->video_info.YResolution * v->video_info.BitsPerPixel) / 8;
  for (int i = 0; i < vram_size; i++) {
    if (v->base_video_address[i] != v->delta_buffer[i]) {
      v->base_video_address[i] = v->delta_buffer[i];
    }
  }
  memset(v->delta_buffer, 0, vram_size);
}
