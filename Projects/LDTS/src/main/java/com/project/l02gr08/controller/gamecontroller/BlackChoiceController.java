...
public class BlackChoiceController implements ChoiceController {
    @Override
    public void step(KeyStroke key, Game game) {
        ...
        switch (key_info) {
            ...
            case Enter:
                PositionState nextState = new PositionState(
                        new BlackPositionController(),
                        new PositionViewer(),
                        board,
                        board.pieceAt(cursorPosition).get()
                );
    ...
    public boolean isValid(KeyType key, ChoiceState state) {
        ...
        switch (key) {
            ...
            case Enter:
                ...
                if (p.isPresent()) {
                    if (p.get().getTeam() == Team.BLACK && !p.get().generateRecommendedMoves(board).isEmpty())
                        return true;
                }
                return false;
