...
public class BlackPositionController implements PositionController {
    ...
    public void step(KeyStroke key, Game game) {
        ...
        switch (key_info) {
            ...
            case Enter:
                ...
                if (board.isCheckMate(Piece.Team.WHITE)) {
                    System.out.println("Black wins!");
                    EndState nextState = new EndState(new EndController(), new EndViewer(), "BLACK");
                    game.setState(nextState);
                } else {
                    ChoiceState nextState = new ChoiceState(new WhiteChoiceController(), new ChoiceViewer(), board);
                    game.setState(nextState);
                }
                return;

            case Escape:
                ChoiceState prevState = new ChoiceState(new BlackChoiceController(), new ChoiceViewer(), board);
    ...
    public Position preferredInitialCursor(Board b) {
        return new Position(b.getBoardLength() - 1, 0);
    }
