...
public class RookTest implements PieceTest {
    ...
    @BeforeEach
    public void setUp() {
        List<Piece> pieces = List.of(new Rook(Piece.Team.WHITE, new Position(3, 3)));
        ...
        for (int i = 0; i < 8; ++i) {
            for (int j = 1; j < 7; ++j) {
                pieces.add(new Rook(Piece.Team.WHITE, new Position(i, j)));
            }
        }
        ...
        for (int i = 0; i < 8; ++i) {
            for (int j = 1; j < 7; ++j) {
                if (i == 3 && j == 3) {
                    pieces.add(new Rook(Piece.Team.BLACK, new Position(i, j)));
                }
                pieces.add(new Rook(Piece.Team.WHITE, new Position(i, j)));
            }
        }
    ...
    public void testInInitialBoard() {
        Piece piece = initialBoard.pieceAt(new Position(0, 0)).get();
