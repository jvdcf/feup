...
@section('content')
    <main class="vh-100 d-flex justify-content-center align-items-center">
        <section class="bg-secondary shadow-lg p-4 w-100 text-white rounded-5" style="max-width: 500px;">
            ...
            <p class="text-center">Please enter your new password for <strong>{{ $email }}</strong></p>


            <form action="{{ route('password.reset.post', ['user' => $user->id]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
